...
| 问题 | 外部触发条件 | 实现 | 验证与复核 |
| --- | --- | --- | --- |
| KubeJS Lazy 并发空缓存 | KubeJS `get()` 与 `forget()` 无同步保护；有效 supplier 也可能返回空缓存，导致资源重载失败。 | `mixin/kubejs/LazyMixin` 通过 `CombatMixinPlugin.postApply` 给两个方法添加 `ACC_SYNCHRONIZED`，不改原方法体。 | 本地 build.24 独立回归；Forge 加载时序、冷启动、重载和服务器回归待测。升级 KubeJS 后复核方法结构与语义，上游修复后评估移除。来源 [CDR #2309](https://github.com/Jasons-impart/Create-Delight-Remake/pull/2309)；命令与 HotAI 迁移见 [专题](kubejs-lazy.md)。 |
