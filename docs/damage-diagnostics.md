...
用户已选择先从极值来源修复：`alexsmobs/WarpedToadPredationDamageMixin` 只替换 Alex's Mobs 1.22.9 蟾蜍 tick 中绯红蚊分支唯一的 `Float.MAX_VALUE` 为 `10_000.0F`。普通攻击仍读取原攻击属性；伤害源、事件、减伤、取消、掉落与死亡流程沿用原逻辑。没有调整暴击范围或增加暴击饱和策略。匹配要求恰好一次，兼容可选模组和 named/SRG 方法名。成功应用时记录 `[CDCore][PredationDamage] Applied ... 10000`，仍须真实启动验证。

10,000 是用户选定的测试值，不代表最高阶段所有装备/减伤组合均已完成一击击杀验证。测试至少覆盖普通捕食、暴击捕食，以及最高阶段正常生成绯红蚊的生存能力；不绕过无敌或事件取消。既有抗性修复继续保留。

探针保留至用户确认本次修复通过后再移除。因 10,000 低于原极值日志阈值，诊断开启时，蟾蜍作为攻击者、绯红蚊作为目标的作用域也保留有限成功样本（`PREDATION_SAMPLE` / `FINAL_TRACE`）；仍按既有 48 项历史与每分钟 20 条跟踪限流。有限样本的 WARN 输出不等于游戏异常，需读取 `firstNonFinite` 和 `ORIGINAL_EXCEPTION`。不要根据限流后没有记录推断所有攻击都正常。

重启后核对 PredationDamage 与 ResistanceDamage 两个成功标记，再验证捕食伤害进入事件为 10,000，默认单次暴击为 15,000，TetraWear/AttributesLib 入参及后续处理均有限，目标正常死亡且 Neruina 无新暂停。已暂停的旧蟾蜍不会自行恢复，需使用正常活动的测试实体或由用户恢复旧实体。
