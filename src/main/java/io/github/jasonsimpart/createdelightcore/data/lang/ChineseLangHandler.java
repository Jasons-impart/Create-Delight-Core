...
public class ChineseLangHandler {
    ...
    private static void addManualOverrides(RegistrateCNLangProvider provider) {
        ...
        replace(provider, "createdelightcore.gui.parser.blank_seal", "空白印章");
        replace(provider, "createdelightcore.gui.parser.blank_clause", "空白订单条款");
        replace(provider, "createdelightcore.gui.parser.clauses", "订单条款");
        replace(provider, "createdelightcore.gui.parser.count_multiplier", "货物数量");
        replace(provider, "createdelightcore.gui.parser.draft", "订单草稿");
        replace(provider, "createdelightcore.gui.parser.entry_count_multiplier", "条目数量");
        replace(provider, "createdelightcore.gui.parser.formal_order", "正式订单");
        replace(provider, "createdelightcore.gui.parser.gift_multiplier", "回礼收益");
        replace(provider, "createdelightcore.gui.parser.insert", "放入订单、草稿或印章");
        replace(provider, "createdelightcore.gui.parser.min_quality_bonus", "质量加成");
        replace(provider, "createdelightcore.gui.parser.money_multiplier", "金币收益");
        replace(provider, "createdelightcore.gui.parser.no_material", "尚未写入印章或条款");
        replace(provider, "createdelightcore.gui.parser.no_data", "没有匹配数据");
        replace(provider, "createdelightcore.gui.parser.no_seal", "尚未盖上方向印章");
        replace(provider, "createdelightcore.gui.parser.order_entries", "需求品类");
        replace(provider, "createdelightcore.gui.parser.order_grade", "订单等级");
        replace(provider, "createdelightcore.gui.parser.owner", "所属玩家");
        replace(provider, "createdelightcore.gui.parser.possible_categories", "可能品类");
        replace(provider, "createdelightcore.gui.parser.possible_customers", "可能顾客");
        replace(provider, "createdelightcore.gui.parser.reputation", "生成声望");
        replace(provider, "createdelightcore.gui.parser.reputation_multiplier", "声望收益");
        replace(provider, "createdelightcore.gui.parser.rule_view", "订单规则");
        replace(provider, "createdelightcore.gui.parser.seal", "印章");
        replace(provider, "createdelightcore.gui.parser.unknown_seal", "未知印章");
        replace(provider, "createdelightcore.gui.parser.unsupported", "不支持的物品");
        replace(provider, "createdelightcore.gui.parser.weight", "权重 %s");
    ...
    public static void init(RegistrateCNLangProvider provider) {
        ...
        provider.add("key.createdelightcore.configuration_module_modifier", "构形模块切换修饰键");
